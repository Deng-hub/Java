package FunctionalInterface.PredicateInterface.Test;
...
请通过Predicate接口的继承将符合要求的字符出筛选到集合ArrayList中，
需要同时满足两个条件：
1.必须为女生
2.姓名为4个字
...
public class Test_Class {
    ...
    public static ArrayList<String> filter(String[] arr, Predicate<String> pre1,Predicate<String> pre2){
        ...
        //遍历数组，获取数组中的每一条信息
        for(String s:arr){
            //使用Predicate接口中的方法test对获取到的字符出进行判断
            boolean b=pre1.and(pre2).test(s);
            //对得到的布尔值进行判读
            if(b){
                //条件成立，连个条件都满足，把信息存储到ArrayList集合中
                list.add(s);
            }
        }
        //把集合返回
